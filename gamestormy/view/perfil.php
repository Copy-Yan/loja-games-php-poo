...
<main class="perfil-wrapper" aria-label="Perfil do usuário">
  ...
  <?php
    ...
    if (authCheck()) {
      $u = new Usuario();
      $isAdmin = (bool) $u->isAdmin($_SESSION['usuario_id'] ?? 0);
    }
  ?>

  <?php if ($isAdmin): ?>
    <div class="perfil-admin-cta" style="margin:14px 0 24px;">
      <a href="<?php echo base_url('?page=admin'); ?>" class="admin-cta-btn" style="display:inline-block;text-decoration:none;background:rgba(0,255,255,.08);border:1px solid rgba(0,255,255,.25);color:var(--cyan);padding:10px 14px;border-radius:12px;font-weight:800;">
        Área do Admin
      </a>
    </div>
  <?php endif; ?>
